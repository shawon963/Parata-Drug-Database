Feature: Measurements Table Data Test between PDDB_LP and PDDB
  Scenario: Verify Measurements Table Data Test between PDDB_LP and PDDB
    ...
    And user get column from "Measurements" table with PDDB database within on prem lan server
    And user count data from "Measurements" table with PDDB database within on prem lan server
