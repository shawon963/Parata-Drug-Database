Feature: DrugAttribute Table Data Test between PDDB_LP and PDDB
  Scenario: Verify DrugAttribute Table Data Test between PDDB_LP and PDDB
    ...
    And user get column from "DrugAttribute" table with PDDB database within on prem lan server
    And user count data from "DrugAttribute" table with PDDB database within on prem lan server
