Feature: DrugCore Table Data Test between PDDB_LP and PDDB
  Scenario: Verify DrugCore Table Data Test between PDDB_LP and PDDB
    ...
    And user get column from "DrugCore" table from PDDB database within on prem lan server
    And user count data from "DrugCore" table from PDDB database within on prem lan server
