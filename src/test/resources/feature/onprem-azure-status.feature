Feature: Status Table Data Test between PDDB_LP and PDDB
  Scenario: Verify Status Table Data Test between PDDB_LP and PDDB
    ...
    And user get column from "Status" table with PDDB database within on prem lan server
    And user count data from "Status" table with PDDB database within on prem lan server
