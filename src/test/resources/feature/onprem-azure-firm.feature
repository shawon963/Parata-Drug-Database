Feature: Firm Table Data Test between PDDB_LP and PDDB
  Scenario: Verify Firm Table Data Test between PDDB_LP and PDDB
    ...
    And user get column from "Firm" table with PDDB database within on prem lan server
    And user count data from "Firm" table with PDDB database within on prem lan server
