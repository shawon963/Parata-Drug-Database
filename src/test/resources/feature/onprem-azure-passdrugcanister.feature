Feature: PassDrugCanister Table Data Test between PDDB_LP and PDDB
  Scenario: Verify PassDrugCanister Table Data Test between PDDB_LP and PDDB
    ...
    And user get column from "PassDrugCanister" table with PDDB database within on prem lan server
    And user count data from "PassDrugCanister" table with PDDB database within on prem lan server
