Feature: ClassificationShape Table Data Test between PDDB_LP and PDDB
  Scenario: Verify ClassificationShape Table Data Test between PDDB_LP and PDDB
    ...
    And user get column from "ClassificationShape" table from PDDB database within on prem lan server
    And user count data from "ClassificationShape" table from PDDB database within on prem lan server
