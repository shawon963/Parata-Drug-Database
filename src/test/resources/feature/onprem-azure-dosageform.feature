Feature: DosageForm Table Data Test between PDDB_LP and PDDB
  Scenario: Verify DosageForm Table Data Test between PDDB_LP and PDDB
    ...
    And user get column from "DosageForm" table from PDDB database within on prem lan server
    And user count data from "DosageForm" table from PDDB database within on prem lan server
