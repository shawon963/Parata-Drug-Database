Feature: StrengthUnitType Table Data Test between PDDB_LP and PDDB
  Scenario: Verify StrengthUnitType Table Data Test between PDDB_LP and PDDB
    ...
    And user get column from "StrengthUnitType" table from PDDB database within on prem lan server
    And user count data from "StrengthUnitType" table from PDDB database within on prem lan server
