Feature: DrugName Table Data Test between PDDB_LP and PDDB
  Scenario: Verify DrugName Table Data Test between PDDB_LP and PDDB
    ...
    And user get column from "DrugName" table from PDDB database within on prem lan server
    And user count data from "DrugName" table from PDDB database within on prem lan server
