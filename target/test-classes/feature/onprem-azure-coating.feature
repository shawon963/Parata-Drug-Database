Feature: Coating Table Data Test between PDDB_LP and PDDB
  Scenario: Verify Coating Table Data Test between PDDB_LP and PDDB
    ...
    And user get column from "Coating" table with PDDB database within on prem lan server
    And user count data from "Coating" table with PDDB database within on prem lan server
