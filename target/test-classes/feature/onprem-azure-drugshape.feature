Feature: DrugShape Table Data Test between PDDB_LP and PDDB
  Scenario: Verify DrugShape Table Data Test between PDDB_LP and PDDB
    ...
    And user get column from "DrugShape" table from PDDB database within on prem lan server
    And user count data from "DrugShape" table from PDDB database within on prem lan server
