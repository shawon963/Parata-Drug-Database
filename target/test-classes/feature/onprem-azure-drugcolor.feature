Feature: DrugColor Table Data Test between PDDB_LP and PDDB
  Scenario: Verify DrugColor Table Data Test between PDDB_LP and PDDB
    ...
    And user get column from "DrugColor" table from PDDB database within on prem lan server
    And user count data from "DrugColor" table from PDDB database within on prem lan server
