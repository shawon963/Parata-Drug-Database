Feature: StatusType Table Data Test between PDDB_LP and PDDB
  Scenario: Verify StatusType Table Data Test between PDDB_LP and PDDB
    ...
    And user get column from "StatusType" table with PDDB database within on prem lan server
    And user count data from "StatusType" table with PDDB database within on prem lan server
