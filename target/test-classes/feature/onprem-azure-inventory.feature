Feature: Inventory Table Data Test between PDDB_LP and PDDB
  Scenario: Verify Inventory Table Data Test between PDDB_LP and PDDB
    ...
    And user get column from "Inventory" table with PDDB database within on prem lan server
    And user count data from "Inventory" table with PDDB database within on prem lan server
