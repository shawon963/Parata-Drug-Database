Feature: DrugNonCountReason Table Data Test between PDDB_LP and PDDB
  Scenario: Verify DrugNonCountReason Table Data Test between PDDB_LP and PDDB
    ...
    And user get column from "DrugNonCountReason" table with PDDB database within on prem lan server
    And user count data from "DrugNonCountReason" table with PDDB database within on prem lan server
