Feature: PassScenarioDrug Table Data Test between PDDB_LP and PDDB
  Scenario: Verify PassScenarioDrug Table Data Test between PDDB_LP and PDDB
    ...
    And user get column from "PassScenarioDrug" table from PDDB database within on prem lan server
    And user count data from "PassScenarioDrug" table from PDDB database within on prem lan server
